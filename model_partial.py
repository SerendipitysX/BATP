...
class Transformer(nn.Module):
    ...
    def __init__(self, time2vector, in_features, out_features, bz, stations):
        ...
        self.encoder = nn.TransformerEncoderLayer(d_model=out_features, nhead=4, dropout=0.1)
...
class LSTM(nn.Module):
    def __init__(self, hidden_dim, out_dim, days, bz, stations):
        super(LSTM, self).__init__()
        self.hidden_dim = hidden_dim
        self.out_dim = out_dim
        assert self.hidden_dim == self.out_dim
        self.embedding = nn.Embedding(bz*stations, self.hidden_dim)
    ...
    def forward(self, x, y):
        ...
        # y [15,1,50] [bz, days(1), stations]  ==> [15,50,1,features]
        y = self.embedding(y.permute(0, 2, 1))
        input = x[:, 0, :, :]  # 15,29,32 [bz,days,features]
        out = torch.zeros_like(x)  # [bz,stations,days,features]
        # fist station
        out[:, 0, :], _ = self.lstm(input)
        # 2ed-Nth station
        for station in range(1, x.shape[1]):
            input = out[:, station-1, :, :] + y[:, :, station-1] + x[:, station, :, :] #[15,29,32]
            out[:, station, :], _ = self.lstm(input)
...
class TemperalModel(nn.Module):
    def __init__(self, Transformer, time2vector, LSTM, bz, stations, days, in_features, out_features, hidden_dim, out_dim):
        ...
        self.L = LSTM(hidden_dim, out_dim, days, bz, stations)
...
# x1 = torch.randn((2, 4, 3)).to(torch.int64)
